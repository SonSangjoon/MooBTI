const resultList = {

    ESFP:{ 
        male:{
            mbti : 'ESFP',
            movie: '타이타닉',
            name: '잭',
            title: '항상 사랑을 말하는 당신은, \n타이타닉의 잭',
            script: '"이 배의 탑승권을 따낸 건 내 인생 최고의 행운이었어. 당신을 만났으니까"',
            description: '외향적이고 감성적인 ESFP 유형의 사람들은 낙천적이고 관계 맺기를 좋아해요. 그래서 연애할 때 상대방에게 사랑을 시도 때도 없이 표현하곤 하죠 😘 \n 종종 연인에게 자신의 가치관을 이야기해요. 상대방이 자신을 그대로 인정해 준다면 온화해지지만, 관계가 조금이라도 불편해진다면 빠르게 거리를 둔답니다. 거부당하는 걸 두려워하는 당신에게, 해주고 싶은 말이 있어요. "사랑하는 사람에게 기꺼이 상처받을 것"이라는 말이에요.',
            movieUrl: 'www.netflix.com/title/1181461',

            imageUrl: '/images/result/jack.png',
            imageSmallUrl: '/images/result/jack_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/FdAgsb.jpg',

            goodMbti: 'ISFJ',
            badMbti:'INFP',
            shortDesc: '항상 사랑을 말하는 남자. 낙천적이고 사교적이지만, 조금이라도 관계가 불편해지면 빠르게 거리를 둬요.',
            
            analysisTitle : "ESFP 잭이 로즈를 사로잡은 비결💘",
            analysisSubTitle : "잭의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '“잭 머릿 속 32명의 기쁨이가 로즈에게 키스 날리는 데에 열일했어요!”\n캐릭터 평균 대비 3명↑)',
            emotion: [32, 21, 14, 9, 24]
           
        },

        female:{
            mbti : 'ESFP',
            movie: '포레스트 검프',
            name: '제니',
            title: '항상 사랑을 말하는 당신은, \n포레스트 검프의 제니',
            script: '"나한테 하나만 약속해 줘. 네가 위험할 때는 용감한 척 말고, 그냥 달려. 그냥 달아나 버려."',
            description: '외향적이고 감성적인 ESFP 유형의 사람들은 낙천적이고 관계 맺기를 좋아해요. 그래서 연애할 때 상대방에게 사랑을 시도 때도 없이 표현하곤 하죠 😘 종종 연인에게 자신의 가치관을 이야기해요. 상대방이 자신을 그대로 인정해준다면 온화해지지만, 관계가 조금이라도 불편해진다면 빠르게 거리를 둔답니다. 거부당하는 걸 두려워하는 당신에게, 해주고 싶은 말이 있어요. "사랑하는 사람에게 기꺼이 상처 받을 것"이라는 말이에요.',
            movieUrl: 'www.netflix.com/title/60000724',

            imageUrl: '/images/result/jenny.png',
            imageSmallUrl: '/images/result/jenny_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/8FGlws.jpg',

            goodMbti: 'ISFJ',
            badMbti: 'INFP' ,
            shortDesc: '항상 사랑을 말하는 여자. \n천적이고 사교적이지만, 조금이라도 관계가 불편해지면 빠르게 거리를 둬요.' , 
            
            analysisTitle : "ESFP 제니가 포레스트 검프를 사로잡은 비결💘",
            analysisSubTitle : "제니의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '“제니 머릿 속 71명의 기쁨이가 애정을 표현하는 데에 열일했어요!”\n캐릭터 평균 대비 42명↑)',
            emotion: [71, 4, 11, 7, 7]
           
        }
    },    
    ISFJ:{ 
        male:{
            mbti : 'ISFJ',
            movie: '포레스트 검프',
            name: '포레스트 검프',
            title: '영원한 사랑을 찾는 당신은,\n포레스트 검프의 포레스트 검프',
            script: '"나, 똑똑하지는 않지만, 사랑이 뭔지는 알아."',
            description: '따뜻하고 배려심 깊은 ISFJ 유형은 상대방을 잘 헤아려요. 그리고 연인과의 사랑의 결실을 꿈꾸기도 하죠. 사랑하는 이에게 의지하면서도, 도움을 주고 싶어 해요. 🧶 소개받는 것보다 자연스럽게 연인과 만나고, 행복한 관계를 오래 유지해요. 그리고 남몰래 사랑의 결실인 결혼을 상상하기도 하죠. 그런데 때론 서운함을 숨기지 않나요? 당신에게 소중한 사람일수록 더 솔직하게 표현하는 건 어때요. 사랑에 무리하지 않는 것", 그것이 당신에게 필요해요.',
            movieUrl: 'www.netflix.com/title/60000724',

            imageUrl: '/images/result/gump.png',
            imageSmallUrl: '/images/result/gump_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/KfNyf4.jpg',

            goodMbti: 'ESFP',
            badMbti:'INFJ',
            shortDesc: '영원한 사랑을 찾는 남자.\n따뜻하고 배려심 넘치지만, 당신에게 서운함이 있어도 마음에 숨겨두고 있어요.',

            analysisTitle : "ISFJ 포레스트 검프가 제니를 사로잡은 비결💘",
            analysisSubTitle : "포레스트 검프의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"검프 머릿 속 43명의 기쁨이가 영원한 사랑을 꿈꾸게 했어요!"\n캐릭터 평균 대비 11명↑)',
            emotion: [43, 13, 16, 9, 19]
           
        },
        female:{
            mbti : 'ISFJ',
            movie: '이터널 션샤인',
            name: '클레멘타인',
            title: '영원한 사랑을 찾는 당신은,\n이터널 션샤인의 클레멘타인',
            script: '"날 기억해 줘. 최선을 다해서... 어쩌면 우린 해낼 수 있을 거야."',
            description: '따뜻하고 배려심 깊은 ISFJ 유형은 상대방을 잘 헤아려요. 그리고 연인과의 사랑의 결실을 꿈꾸기도 하죠. 사랑하는 이에게 의지하면서도, 도움을 주고 싶어 해요. 🧶 소개받는 것보다 자연스럽게 연인과 만나고, 행복한 관계를 오래 유지해요. 그리고 남몰래 사랑의 결실인 결혼을 상상하기도 하죠. 그런데 때론 서운함을 숨기지 않나요? 당신에게 소중한 사람일수록 더 솔직하게 표현하는 건 어때요. 사랑에 무리하지 않는 것", 그것이 당신에게 필요해요.',
            movieUrl: 'www.watcha.com/contents/m85X9LW',

            imageUrl: '/images/result/clementine.png',
            imageSmallUrl: '/images/result/clementine_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/QzV9Es.jpg',

            goodMbti: 'ESFP',
            badMbti: 'INFJ' ,
            shortDesc: '영원한 사랑을 찾는 여자.\n따뜻하고 배려심 넘치지만, 당신에게 서운함이 있어도 마음에 숨겨두고 있어요.' , 
        
            analysisTitle : "ISFJ 클레멘타인이 조엘을 사로잡은 비결💘",
            analysisSubTitle : "클레멘타인의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"클레멘타인 머릿 속 17명의 까칠이가 감정에 솔직한 치명적 매력을 만들었어요."\n캐릭터 평균 대비 5명↑)',
            emotion: [27, 20, 13, 17, 23]
           
        }
    },
    INFP:{ 
        male:{
            mbti : 'INFP',
            movie: '프렌즈 위드 베네핏',
            name: '딜런',
            title: '낭만적 사랑을 꿈꾸는 당신은,\n프렌즈 위드 베네핏의 딜런',
            script: '“사랑하는 사람과는 금요일 밤만이 아니라, 토요일도 하루 종일 함께 하고 싶다는 거야.”',
            description: '이상적이고 낭만적인 사랑을 꿈꾸는 INFP 유형은, 내면에서 혼자 많은 고민을 해요. ‘사람들과 만났을 때 어떤 얘기를 하지?’ ‘그에게 내가 좋아하는 걸 들키면 어떡하지?’를 미리 고민한 적 있다는 거, 다 알아요 🤭 처음엔 낯을 가려도 나중엔 속마음을 털어놓아요. 아니 어느샌가 연인에게 환상을 갖고 콩깍지가 아주 제대로 씌어 버리죠. 그래서 조심해야 해요. 냉정한 현실주의자(ST 유형)와 연애할 때 마음이 많이 아플 거라서요. 내 감정을 잘 못 읽더라도 연인이 당신을 사랑하지 않는다고 생각하지 말아요. 그는 사랑하고 있어요.',
            movieUrl: 'www.netflix.com/title/70167075',

            imageUrl: '/images/result/dylan.png',
            imageSmallUrl: '/images/result/dylan_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/u8tWPH.jpg',

            goodMbti: 'ENTJ',
            badMbti:'ISFP',
            shortDesc: '낭만적 사랑을 찾아다니는 남자.\n내면에서 혼자 고민을 많이 하지만, 당신에게 환상을 갖고 푹 빠져버릴 거예요.',

            analysisTitle : "INFP 딜런이 제이미를 사로잡은 비결💘",
            analysisSubTitle : "딜런의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"딜런 머릿 속 32명의 기쁨이가 제이미에게 푹 빠지도록 만들어요."\n캐릭터 평균 대비 4명↑)',
            emotion: [32, 15, 20, 16, 17]
           
        },
        female:{
            mbti : 'INFP',
            movie: '내.사.모.남',
            name: '라라진',
            title: '낭만적 사랑을 꿈꾸는 당신은,\n내사모남의 라라진',
            script: '“내가 조쉬를 좋아하지 않는 것처럼 보이기 위해선, 너에게 푹 빠진 사람처럼 보여야 해.”',
            description: '이상적이고 낭만적인 사랑을 꿈꾸는 INFP 유형은, 내면에서 혼자 많은 고민을 해요. ‘사람들과 만났을 때 어떤 얘기를 하지?’ ‘그에게 내가 좋아하는 걸 들키면 어떡하지?’를 미리 고민한 적 있다는 거, 다 알아요 🤭 처음엔 낯을 가려도 나중엔 속마음을 털어놓아요. 아니 어느샌가 연인에게 환상을 갖고 콩깍지가 아주 제대로 씌어 버리죠. 그래서 조심해야 해요. 냉정한 현실주의자(ST 유형)와 연애할 때 마음이 많이 아플 거라서요. 내 감정을 잘 못 읽더라도 연인이 당신을 사랑하지 않는다고 생각하지 말아요. 그는 사랑하고 있어요.',
            movieUrl: 'www.netflix.com/title/80203147',

            imageUrl: '/images/result/lara.png',
            imageSmallUrl: '/images/result/lara_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/IthhDQ.jpg',

            goodMbti: 'ENTJ',
            badMbti: 'ISFP' ,
            shortDesc: '낭만적 사랑을 찾아다니는 여자.\n내면에서 혼자 고민을 많이 하지만, 당신에게 환상을 갖고 푹 빠져버릴 거에요.' , 
        
            analysisTitle : "INFP 라라진이 피터를 사로잡은 비결💘",
            analysisSubTitle : "라라진의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"라라진 머릿 속 32명의 기쁨이가 피터에게 푹 빠지도록 만들어요."\n캐릭터 평균 대비 4명↑)',
            emotion: [32, 22, 13, 12, 21]
           
        }
    },
    ISTJ:{ 
        male:{
            mbti : 'ISTJ',
            movie: '인셉션',
            name: '아서',
            title: '믿음직한 연인이 되어줄 당신은,\n인셉션의 아서',
            script: '"빨리, 나한테 키스해."',
            description: '신중하고 철저한 ISTJ 유형은 자신만의 바운더리가 뚜렷하고, 연애보다 일과 공부를 우선시해요. 자기관리를 잘 하는 모습에 매력을 느끼는 사람들이 많죠. 😘 신뢰를 중요시하기 때문에 믿음직한 연인이 되어줄거예요. 눈치가 빠르고 사소한 것도 잘 기억해서 연인을 세심하게 챙겨주기도 하고요! (그 애인 참 부럽다...) 하지만 자신만의 시간이 중요하고 외로움을 잘 몰라서, 연애를 시작하는 게 어렵지 않으신가요? 만약 누군가 마음속에 훅 들어온다면, 조금은 과감하게 다가가보세요. 그 사람도 당신을 기다리고 있을지 몰라요!',
            movieUrl: 'www.netflix.com/title/70131314',

            imageUrl: '/images/result/arthur.png',
            imageSmallUrl: '/images/result/arthur_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/NkTq6U.jpg',

            goodMbti: 'ESFP',
            badMbti:'ENFJ',
            shortDesc: '믿음직한 연인이 되어줄 남자. \n쉽게 마음을 얻을 수 없지만, 한 번 사랑에 빠지면 오래 사랑을 나눌 거예요.',

            analysisTitle : "ISTJ 아서가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "아서의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"아서 머릿 속 균일한 감정 친구들이 감정 동요 없는 믿음직한 모습을 만들어요"',
            emotion: [18, 25, 20, 14, 23]
           
        },
        female:{
            mbti : 'ISTJ',
            movie: '내.사.모.남',
            name: '마곳 코비',
            title: '믿음직한 연인이 되어줄 당신은,\n내사모남의 마곳 코비',
            script: '"라라 진, 난 내일 떠나. 그건 네가 큰언니가 된다는 뜻이야."',
            description: '신중하고 철저한 ISTJ 유형은 자신만의 바운더리가 뚜렷하고, 연애보다 일과 공부를 우선시해요. 자기관리를 잘 하는 모습에 매력을 느끼는 사람들이 많죠. 😘 신뢰를 중요시하기 때문에 믿음직한 연인이 되어줄거예요. 눈치가 빠르고 사소한 것도 잘 기억해서 연인을 세심하게 챙겨주기도 하고요! (그 애인 참 부럽다...) 하지만 자신만의 시간이 중요하고 외로움을 잘 몰라서, 연애를 시작하는 게 어렵지 않으신가요? 만약 누군가 마음속에 훅 들어온다면, 조금은 과감하게 다가가보세요. 그 사람도 당신을 기다리고 있을지 몰라요!',
            movieUrl: 'www.netflix.com/title/80203147',
            
            imageUrl: '/images/result/covey.png',
            imageSmallUrl: '/images/result/covey_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/YmnxaF.jpg',

            goodMbti: 'ESFP',
            badMbti:'ENFJ',
            shortDesc: '믿음직한 연인이 되어줄 여자. \n쉽게 마음을 얻을 수 없지만, 한 번 사랑에 빠지면 오래 사랑을 나눌 거예요.' , 
        
            analysisTitle : "ISTJ 마곳코비가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "마곳코비의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"마곳코비 머릿 속 균일한 감정 친구들이 감정 동요 없는 믿음직한 모습을 만들어요"',
            emotion: [31, 17, 10, 21, 21]
           
        }
    },
    ISTP:{ 
        male:{
            mbti : 'ISTP',
            movie: '브리저튼',
            name: '사이먼',
            title: '남몰래 사랑에 몰입하는 당신은,\n브리저튼의 사이먼',
            script: '"이제 다프네의 행복은 내가 책임져. 난 그걸 최우선으로 여기네.”',
            description: '내 사람에게만 몰입하는 ISTP 유형은, 혼자만의 시간을 굉장히 중요하게 여깁니다. 그리고 좋아하는 감정을 잘 표현하지 않죠. 하지만 사랑하는 사람과 있을 때면 얘기는 달라요. 그 순간에는 한 사람에게만 몰두하니까요. 💘 ISTP 유형은 자유로운 연애를 지향해요. 취미와 흥미를 위해 개인적인 시간을 가져야 하고, 본인 취미에 애인이 함께 하기를 원해요. 가끔은 연락이 뜸하기도 하는데, 별 뜻은 없어요. 그런데 애인이 이런 걸로 토라질 때가 많죠? 억울해도 표현하지 않으면 상대방은 잘 몰라요. 당신이 그만을 바라보고 있다는걸요. 지금 당장 사랑한다고 말해보는 건 어때요?',
            movieUrl: 'www.netflix.com/title/80232398',

            imageUrl: '/images/result/simon.png',
            imageSmallUrl: '/images/result/simon_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/btYeI0.jpg',

            goodMbti: 'ESFJ',
            badMbti:'ENFP',
            shortDesc: '내색 없이 사랑에 몰입하는 남자.\n자유로워 보이지만 속으로는 오직 당신만 바라보고 있어요.',

            analysisTitle : "ISTP 사이먼이 다프네를 사로잡은 비결💘",
            analysisSubTitle : "사이먼의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"사이먼 머릿 속 33명의 기쁨이가 다프네만을 바라보게 만들어요."\n캐릭터 평균 대비 4명↑)',
            emotion: [33, 16, 14, 12, 25]
           
        },
        female:{
            mbti : 'ISTP',
            movie: '아메리칸 파이',
            name: '제시카',
            title: '남몰래 사랑에 몰입하는 당신은,\n아메리칸 파이의 제시카',
            script: '“그냥 그를 사랑한다고 말해. 그게 바로 내가 넘어간 이유였어.”',
            description: '내 사람에게만 몰입하는 ISTP 유형은, 혼자만의 시간을 굉장히 중요하게 여깁니다. 그리고 좋아하는 감정을 잘 표현하지 않죠. 하지만 사랑하는 사람과 있을 때면 얘기는 달라요. 그 순간에는 한 사람에게만 몰두하니까요. 💘 ISTP 유형은 자유로운 연애를 지향해요. 취미와 흥미를 위해 개인적인 시간을 가져야 하고, 본인 취미에 애인이 함께 하기를 원해요. 가끔은 연락이 뜸하기도 하는데, 별 뜻은 없어요. 그런데 애인이 이런 걸로 토라질 때가 많죠? 억울해도 표현하지 않으면 상대방은 잘 몰라요. 당신이 그만을 바라보고 있다는걸요. 지금 당장 사랑한다고 말해보는 건 어때요?',
            movieUrl: 'www.watcha.com/contents/share/m5NE18d',

            imageUrl: '/images/result/jessica.png',
            imageSmallUrl: '/images/result/jessica_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/p0vip6.jpg',

            goodMbti: 'ESFJ',
            badMbti:'ENFP',
            shortDesc: '내색 없이 사랑에 몰입하는 여자. \n자유로워 보이지만 속으로는 오직 당신만 바라보고 있어요.' , 
        
            analysisTitle : "ISTP 제시카가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "제시카의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"제시카 머릿 속 46명의 기쁨이가 오직 사랑할 궁리만 생각하고 있어요."\n캐릭터 평균 대비 17명↑)',
            emotion: [46, 18, 12, 12, 12]
           
        }
    },
    ESTP:{ 
        male:{
            mbti : 'ESTP',
            movie: '아메리칸 파이',
            name: '스티브',
            title: '사랑에 전력 질주하는 당신은,\n아메리칸 파이의 스티브',
            script: '"와! 같은 책을 좋아하는 우리는 정말 닮은 게 많구나!"',
            description: '외향적이고 과감한 ESTP 유형은, 계산적이지 않고 솔직한 성격을 갖고 있어요. 어떤 사람이 마음에 들어오는 순간 오직 그를 위해 최선을 다해 달려가죠. 🏃 ♂️ 연애 초기 당신의 사랑은 매우 로맨틱해요. 항상 연인에게 사랑을 적극적으로 표현하는데, 집착도 하지 않으니까요. 하지만 연애가 편해질 때, 항상 고민하게 되지 않던가요? 뜨겁지 않은 사랑도 사랑인지를요. 그 와중에 애인은 ‘밀당’하는 줄 알고 미쳐버리곤 하죠. 하지만 고민에 너무 빠지진 말아요. 익숙함 때문에 무뎌진 사랑도, 사랑이니까요. 당신은 그를 여전히 사랑하고 있어요.',
            movieUrl: 'www.watcha.com/contents/share/m5NE18d',
            
            imageUrl: '/images/result/steve.png',
            imageSmallUrl: '/images/result/steve_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/cdTg57.jpg',

            goodMbti: 'ISFJ',
            badMbti:'ENFJ',
            shortDesc: '사랑을 위해 전력 질주하는 남자.\n하지만 그의 마음은 주전자 같아서, 당신의 마음을 애태울 거예요.',

            analysisTitle : "ESTP 스티브가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "스티브의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"스티브 머릿 속 22명의 버럭이가 사랑에 질주하는 당신을 만들어요."\n캐릭터 평균 대비 6명↑)',
            emotion: [27, 17, 22, 17, 17]
           
        },
        female:{
            mbti : 'ESTP',
            movie: '프렌즈 위드 베네핏',
            name: '제이미',
            title: '사랑에 전력 질주하는 당신은,\n프렌즈 위드 베네핏의 제이미',
            script: '"네 인생을 바꿔줄게. 내가 그 여자야."',
            description: '외향적이고 과감한 ESTP 유형은, 계산적이지 않고 솔직한 성격을 갖고 있어요. 어떤 사람이 마음에 들어오는 순간 오직 그를 위해 최선을 다해 달려가죠. 🏃 ♂️ 연애 초기 당신의 사랑은 매우 로맨틱해요. 항상 연인에게 사랑을 적극적으로 표현하는데, 집착도 하지 않으니까요. 하지만 연애가 편해질 때, 항상 고민하게 되지 않던가요? 뜨겁지 않은 사랑도 사랑인지를요. 그 와중에 애인은 ‘밀당’하는 줄 알고 미쳐버리곤 하죠. 하지만 고민에 너무 빠지진 말아요. 익숙함 때문에 무뎌진 사랑도, 사랑이니까요. 당신은 그를 여전히 사랑하고 있어요.',
            movieUrl: 'www.netflix.com/title/70167075',

            imageUrl: '/images/result/jamie.png',
            imageSmallUrl: '/images/result/jamie_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/C44LYR.jpg',

            goodMbti: 'ISFJ',
            badMbti:'ENFJ',
            shortDesc: '사랑을 위해 전력 질주하는 여자. \n하지만 그의 마음은 주전자 같아서, 당신의 마음을 애태울 거예요.' , 
        
            analysisTitle : "ESTP 제이미가 딜런을 사로잡은 비결💘",
            analysisSubTitle : "제이미의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"제이미 머릿 속 21명의 버럭이가 사랑에 질주하는 당신을 만들어요."\n캐릭터 평균 대비 5명↑)',
            emotion: [24, 17, 21, 19, 19]
           
        }
    },
    ESTJ:{ 
        male:{
            mbti : 'ESTJ',

            movie: '반지의 제왕',
            name: '보로미르',
            title: '안정적인 사랑을 고수하는 당신은,\n반지의제왕의 보로미르',
            script: '"나는 끝까지 따랐을 걸세. 내 형제여, 내 대장이여, 나의 왕이시여."',
            description: '헌신적이고 책임감 강한 ESTJ 유형은, 안정적인 사랑을 위해 노력해요. 좋아하는 사람을 위해 언제든 맞춰줄 준비가 되어 있죠. 바람피우는 일은 절대 없을 거고요.🤭 ESTJ에게 사랑은 변하지 않는 거예요. 이런 사랑을 이뤄내기 위해 사소한 다툼을 피하기 위해 노력해요. 한편 자신처럼 애인도 관계에 헌신하기를 바라죠. 이러한 기대감에 가끔 애인에게 크게 실망할 수도 있답니다. 당신을 위해 지속가능한 관계의 비결을 알려줄게요. 비결은 무리하지 않는 것이에요. 헌신의 무게감을 버리고 본인의 감정에 집중하고 표현하세요. 사랑하는 마음만 있으면 돼요.',
            movieUrl: 'www.watcha.com/contents/mP5mPzd',
            
            imageUrl: '/images/result/boromir.png',
            imageSmallUrl: '/images/result/boromir_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/gOyXYF.jpg',

            goodMbti: 'ENTJ',
            badMbti:'INFP',
            shortDesc: '사랑을 위해 헌신하는 남자.\n당신에게도 헌신을 기대하는 게 문제가 되기도 해요.',

            analysisTitle : "ESTJ 보로미르가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "보로미르의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"보로미르 머릿 속 28명의 슬픔이가 슬픔 없는 안정적 사랑을 만들려고 노력해요."\n캐릭터 평균 대비 8명↑)',
            emotion: [12, 28, 21, 14, 25]
           
        },
        female:{
            mbti : 'ESTJ',
            movie: '해리포터',
            name: '헤르미온느',
            title: '안정적인 사랑을 고수하는 당신은,\n해리포터의 헤르미온느',
            script: '"다음 무도회 때에는 다른 사람이 나한테 신청하기 전에 먼저 나한테 신청하도록 해!"',
            description: '헌신적이고 책임감 강한 ESTJ 유형은, 안정적인 사랑을 위해 노력해요. 좋아하는 사람을 위해 언제든 맞춰줄 준비가 되어 있죠. 바람피우는 일은 절대 없을 거고요.🤭 ESTJ에게 사랑은 변하지 않는 거예요. 이런 사랑을 이뤄내기 위해 사소한 다툼을 피하기 위해 노력해요. 한편 자신처럼 애인도 관계에 헌신하기를 바라죠. 이러한 기대감에 가끔 애인에게 크게 실망할 수도 있답니다. 당신을 위해 지속가능한 관계의 비결을 알려줄게요. 비결은 무리하지 않는 것이에요. 헌신의 무게감을 버리고 본인의 감정에 집중하고 표현하세요. 사랑하는 마음만 있으면 돼요.',
            movieUrl: 'www.watcha.com/contents/mmOb3kd',

            imageUrl: '/images/result/hermione.png',
            imageSmallUrl: '/images/result/hermione_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/RDZjrm.jpg',

            goodMbti: 'ENTJ',
            badMbti:'INFP',
            shortDesc: '사랑을 위해 헌신하는 여자.\n당신에게도 헌신을 기대하는 게 문제가 되기도 해요.' , 

            analysisTitle : "ESTJ 헤르미온느가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "헤르미온느의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"헤르미온느 머릿 속 21명의 버럭이가 화가 없는 안정적 사랑을 만들려고 노력해요."\n캐릭터 평균 대비 5명↑)',
            emotion: [18, 18, 21, 17, 26]
           
        }
    },
    ISFP:{ 
        male:{
            mbti : 'ISFP',
            movie: '라라랜드',
            name: '세바스찬',
            title: '호수같은 사랑을 꿈꾸는 당신은,\n라라랜드의 세바스찬',
            script: '"그냥 흘러가는 대로 두고 보자."',
            description: '자유롭고 즉흥적인 ISFP 유형은, 평화를 사랑하고 갈등을 싫어해요. 그래서 고백을 거절하기도, 받아들이기도 어려워하죠. 하지만 배려심 깊고 겸손해서, 이들이 포근한 매력에 빠지는 사람이 많아요. 🧸 당신은 연애에 신중하기 때문에 관계를 주도하는 스타일은 아니군요! 상대의 리드에 몸을 맡기는 당신은, 불도저 같은 뜨거운 스타일과 잘 어울려요. 생각이 많지만 귀차니즘이 심하기 때문에, 마음 속 이야기를 털어놓지를 못하네요. 가끔은 솔직하게 그에게 숨긴 얘기를 풀어 볼까요? 연인과 한층 더 가깝고 뜨거운 사이가 될 거예요.',
            movieUrl: 'www.watcha.com/contents/m5GXz42',

            imageUrl: '/images/result/sebastian.png',
            imageSmallUrl: '/images/result/sebastian_small.png',  
            kakaoImageUrl: 'https://ifh.cc/g/NcDgCd.jpg',

            goodMbti: 'ESTJ',
            badMbti:'INFJ',
            shortDesc: '호수같은 사랑을 꿈꾸는 남자. \n귀차니즘이 심한 그는 당신의 적극적인 사랑 고백을 기다리고 있어요!',

            analysisTitle : "ISFP 세바스찬이 미아를 사로잡은 비결💘",
            analysisSubTitle : "세바스찬의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"세바스찬 머릿 속 42명의 기쁨이가 불도저 그녀에게 마음을 뛰게 만들어요."\n캐릭터 평균 대비 13명↑)',
            emotion: [42, 14, 15, 14, 15]
           

        },
        female:{
            mbti : 'ISFP',
            movie: '트와일라잇',
            name: '벨라',
            title: '호수같은 사랑을 꿈꾸는 당신은,\n트와일라잇의 벨라',
            script: '"너를 두려워하지 않아. 단지 널 잃는 게 두려울 뿐이야."',
            description: '자유롭고 즉흥적인 ISFP 유형은, 평화를 사랑하고 갈등을 싫어해요. 그래서 고백을 거절하기도, 받아들이기도 어려워하죠. 하지만 배려심 깊고 겸손해서, 이들이 포근한 매력에 빠지는 사람이 많아요. 🧸 당신은 연애에 신중하기 때문에 관계를 주도하는 스타일은 아니군요! 상대의 리드에 몸을 맡기는 당신은, 불도저 같은 뜨거운 스타일과 잘 어울려요. 생각이 많지만 귀차니즘이 심하기 때문에, 마음 속 이야기를 털어놓지를 못하네요. 가끔은 솔직하게 그에게 숨긴 얘기를 풀어 볼까요? 연인과 한층 더 가깝고 뜨거운 사이가 될 거예요.',
            movieUrl: 'www.netflix.com/title/70099113',
            
            imageUrl: '/images/result/bella.png',
            imageSmallUrl: '/images/result/bella_small.png',                        
            kakaoImageUrl: 'https://ifh.cc/g/JHwbC5.jpg',

            goodMbti: 'ESTJ',
            badMbti:'INFJ',
            shortDesc: '호수같은 사랑을 꿈꾸는 여자. \n귀차니즘이 심한 그는 당신의 적극적인 사랑 고백을 기다리고 있어요!' , 

            analysisTitle : "ISFP 벨라가 에드워드를 사로잡은 비결💘",
            analysisSubTitle : "벨라의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"벨라 머릿 속 26명의 소심이가 불도저 그에게 마음을 뛰게 만들어요."\n캐릭터 평균 대비 6명↑)',
            emotion: [27, 24, 16, 7, 26]
           
        }
    },
    ESFJ:{ 
        male:{
            mbti : 'ESFJ',
            movie: '테드',
            name: '존',
            title: '마음 속 사랑을 가득 채운 당신은,\n테드의 존',
            script: '"사랑해 테드. 네가 정말로 말을 할 줄 알면 좋겠어. 그럼 우린 영원한 단짝 친구가 될 수 있을 텐데"',
            description: '주도적이며 대화를 즐기는 ESFJ 유형은 본투비 주인공이에요. 상황이 자기 중심으로 돌아가는 걸 좋아하죠. 외향적인 성격에 사람들이 자신에게 호기심을 가지길 바라요. 🕺🏻 다정한 당신은 애인에게 관심과 사랑을 아주 퍼부어 주네요. 현실적인 성향이 강해서 상대와 자신의 상황과 조건, 성격을 꼼꼼히 따지며 고민하지만 연애를 시작하면 그에게 헌신하는 스타일! 그리고 계획적 데이트와 안정적 관계를 선호하는군요. 그래서 그런 걸까요? 애인에게 준 사랑만큼 사랑받지 못하면 상처를 받아요. 하지만 걱정하지 말아요. 나중엔 그가 당신에게 더 푹 빠질 거니까요.',
            movieUrl: 'www.netflix.com/title/70218756',

            imageUrl: '/images/result/ted.png',
            imageSmallUrl: '/images/result/ted_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/RwwRt5.jpg',

            goodMbti: 'ISTP',
            badMbti:'ENFJ',
            shortDesc: '마음속에 사랑을 가득 채운 남자.\n인기가 많은 그의 마음을 가지려면, 조급하겠는데요?',

            analysisTitle : "ESFJ 존이 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "존의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"존 머릿 속 21명의 버럭이가 사랑 표현을 애인에게 퍼부어 줘요."\n캐릭터 평균 대비 5명↑)',
            emotion: [29, 17, 21, 13, 20]
           

        },
        female:{
            mbti : 'ESFJ',
            movie: '브리저튼',
            name: '다프네',
            title: '마음 속에 사랑을 채운 당신은,\n브리저튼의 다프네',
            script: '"저는 당신을 갈망해요..!"',
            description: '주도적이며 대화를 즐기는 ESFJ 유형은 본투비 주인공이에요. 상황이 자기 중심으로 돌아가는 걸 좋아하죠. 외향적인 성격에 사람들이 자신에게 호기심을 가지길 바라요. 🕺🏻 다정한 당신은 애인에게 관심과 사랑을 아주 퍼부어 주네요. 현실적인 성향이 강해서 상대와 자신의 상황과 조건, 성격을 꼼꼼히 따지며 고민하지만 연애를 시작하면 그에게 헌신하는 스타일! 그리고 계획적 데이트와 안정적 관계를 선호하는군요. 그래서 그런 걸까요? 애인에게 준 사랑만큼 사랑받지 못하면 상처를 받아요. 하지만 걱정하지 말아요. 나중엔 그가 당신에게 더 푹 빠질 거니까요.',
            movieUrl: 'www.netflix.com/title/80232398',

            imageUrl: '/images/result/dafne.png',
            imageSmallUrl: '/images/result/dafne_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/JZm97T.jpg',

            goodMbti: 'ISTP',
            badMbti:'ENFJ',
            shortDesc: '마음속에 사랑을 가득 채운 여자. \n인기가 많은 그의 마음을 가지려면, 조급하겠는데요?',

            analysisTitle : "ESFJ 다프네가 사이먼을 사로잡은 비결💘",
            analysisSubTitle : "다프네의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"다프네 머릿 속 41명의 기쁨이가 그에게 미친듯이 가슴 뛰게 해요."\n캐릭터 평균 대비 12명↑)',
            emotion: [41, 23, 10, 11, 15]

        }
    },
    INFJ:{ 
        male:{
            mbti : 'INFJ',
            movie: '트와일라잇',
            name: '에드워드',
            title: '사랑에 주관이 뚜렷한 당신은,\n트와일라잇의 에드워드',
            script: '"너를 멀리하려고 애썼는데, 지쳐버린 것 같아."',
            description: "관찰력이 뛰어난 INFJ 유형은 눈치가 빠르고, 가식을 쉽게 분별해요. 하지만 갈등을 싫어하기 때문에 모른 척하는 경우가 많죠. 호기심 강하고 독립적이며 주관이 뚜렷한 당신은, 자신의 신념을 밀고 나가요. 🚀  \n\n 기본적으로 친절하고 상냥한 INFJ 유형은, 어른스럽고 온화해서 애인에게 먼저 배려하고 양보해요. 하지만 자신만의 선이 뚜렷해서 선 넘는 행동을 하면 관계를 정리하죠. 그런데 당신은 좋아하는 사람에게 먼저 다가가지 않는 타입이네요! 가끔은 기다리기보다는 먼저 다가가서 사랑을 쟁취해보는 건 어때요? 당신의 색다른 모습에 반할 거예요.",
            movieUrl: 'www.netflix.com/title/70099113',

            imageUrl: '/images/result/edward.png',
            imageSmallUrl: '/images/result/edward_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/ilkNhn.jpg',

            goodMbti: 'ENTP',
            badMbti:'ISTJ',
            shortDesc: '주관이 뚜렷하고 독립심 강한 남자. \n 그가 당신을 좋아하는 걸 숨기고 있어요. 먼저 어필해볼까요?',

            analysisTitle : "INFJ 에드워드가 벨라를 사로잡은 비결💘",
            analysisSubTitle : "에드워드의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"에드워드 머릿 속 33명의 소심이가 그녀에게 조심스럽게 다가가게 해요."\n캐릭터 평균 대비 13명↑)',
            emotion: [22, 22, 13, 10, 33]
           

        },
        female:{
            mbti : 'INFJ',
            movie: '헤더스',
            name: '베로니카',
            title: '사랑에 주관이 뚜렷한 당신은,\n헤더스의 베로니카',
            script: '"내가 인간처럼 살기엔 잘못된 시대에 태어났나 봐."',
            description: '관찰력이 뛰어난 INFJ 유형은 눈치가 빠르고, 가식을 쉽게 분별해요. 하지만 갈등을 싫어하기 때문에 모른 척하는 경우가 많죠. 호기심 강하고 독립적이며 주관이 뚜렷한 당신은, 자신의 신념을 밀고 나가요. 🚀 기본적으로 친절하고 상냥한 INFJ 유형은, 어른스럽고 온화해서 애인에게 먼저 배려하고 양보해요. 하지만 자신만의 선이 뚜렷해서 선 넘는 행동을 하면 관계를 정리하죠. 그런데 당신은 좋아하는 사람에게 먼저 다가가지 않는 타입이네요! 가끔은 기다리기보다는 먼저 다가가서 사랑을 쟁취해보는 건 어때요? 당신의 색다른 모습에 반할 거예요.',
            movieUrl: 'www.watcha.com/contents/mqW9045',

            imageUrl: '/images/result/veronica.png',
            imageSmallUrl: '/images/result/veronica_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/PJ5tPT.jpg',

            goodMbti: 'ENTP',
            badMbti:'ISTJ',
            shortDesc: '주관이 뚜렷하고 독립심 강한 여자.\n그가 당신을 좋아하는 걸 숨기고 있어요. 먼저 어필해볼까요?' , 

            analysisTitle : "INFJ 베로니카가 J.D.을 사로잡은 비결💘",
            analysisSubTitle : "베로니카의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"베로니카 머릿 속 22명의 소심이가 그녀에게 조심스럽게 다가가게 해요."\n캐릭터 평균 대비 2명↑)',
            emotion: [22, 20, 21, 15, 22]
           
        }
    },
    ENFP:{ 
        male:{
            mbti : 'ENFP',
            movie: '이터널 션샤인',
            name: '조엘',
            title: '여린 마음으로 사랑하는 당신은,\n이터널 션샤인 조엘',
            script: '"제발 이 기억만은 남겨주세요. 이 순간만큼은."',
            description: '활발하고 사교적인 ENFP 유형은, 새로운 사람들과의 만남을 즐겨요. 사람들의 관심과 애정을 아주 좋아하죠. 또한 쉽게 지루함을 느끼고 즉흥적인 면이 있어요. 🕹 어렵지 않게 사람들에게 다가가고, 호감이 가는 사람에게 직진하는 성향을 가졌기 때문에 당신이 연애를 시작하는 계기가 되는 경우가 많죠. 관심 없는 사람에게도 매력을 흘려, 착각해서 다가오는 사람들로 난감해질 수도 있어요. 혹시 인간관계 스트레스에 취약하지 않나요? 밝고 외향적인 외면에 가려진 여린 마음이 있으니까요. 그래서 속 깊은 부분까지 감싸줄 수 있는 사람을 만나서 행복해야 한답니다!',
            movieUrl: 'www.watcha.com/contents/m85X9LW',
            
            imageUrl: '/images/result/joel.png',
            imageSmallUrl: '/images/result/joel_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/jwWhIh.jpg',

            goodMbti: 'INFJ',
            badMbti:'ESTP',
            shortDesc: '순수하고 사교성이 넘치는 남자.\n당신의 애정으로 그의 여린 내면까지 포용해 주세요.',

            analysisTitle : "ENFP 조엘이 클레멘타인을 사로잡은 비결💘",
            analysisSubTitle : "조엘의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"조엘 머릿속 24명의 슬픔이가 그녀를 향한 여린 마음을 만들었어요."\n캐릭터 평균 대비 4명↑)',
            emotion: [25, 24, 14, 17, 20]
           

        },
        female:{
            mbti : 'ENFP',
            movie: '브리짓 존스의 일기',
            name: '브리짓 존스',
            title: '여린 마음으로 사랑하는 당신은, \n브리짓 존스의 일기의 브리짓',
            script: '"당신 말대로, 나도 여전히 특별한 상대를 찾고 있어요."',
            description: '활발하고 사교적인 ENFP 유형은, 새로운 사람들과의 만남을 즐겨요. 사람들의 관심과 애정을 아주 좋아하죠. 또한 쉽게 지루함을 느끼고 즉흥적인 면이 있어요. 🕹 어렵지 않게 사람들에게 다가가고, 호감이 가는 사람에게 직진하는 성향을 가졌기 때문에 당신이 연애를 시작하는 계기가 되는 경우가 많죠. 관심 없는 사람에게도 매력을 흘려, 착각해서 다가오는 사람들로 난감해질 수도 있어요. 혹시 인간관계 스트레스에 취약하지 않나요? 밝고 외향적인 외면에 가려진 여린 마음이 있으니까요. 그래서 속 깊은 부분까지 감싸줄 수 있는 사람을 만나서 행복해야 한답니다!',
            movieUrl: 'www.netflix.com/kr/title/60020322',
            
            imageUrl: '/images/result/bridget.png',
            imageSmallUrl: '/images/result/bridget_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/Ij4PtM.jpg',

            goodMbti: 'INFJ',
            badMbti:'ESTP',
            shortDesc: '순수하고 사교성이 넘치는 여자. \n당신의 애정으로 그의 여린 내면까지 포용해 주세요.' , 

            analysisTitle : "ENFP 브리짓이 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "브리짓의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"브리짓 머릿속 22명의 슬픔이가 그를 향한 여린 마음을 만들었어요."\n캐릭터 평균 대비 2명↑)',
            emotion: [24, 22, 17, 18, 19]
           
        }
    },
    ENFJ:{ 
        male:{
            mbti : 'ENFJ',
            movie: '내.사.모.남',
            name: '피터',
            title: '센스 넘치는 사랑꾼 당신은,\n내사모남의 피터',
            script: '"왜냐하면 내가 너를 사랑하게 되었기 때문이야. 오직 너만을."',
            description: '솔직하고 센스 있는 ENFJ 유형은, 호불호가 명확한 편이에요. 말을 잘하고 부드러운 리더십도 있어 주위에 사람들이 많이 모이죠. 효율적인 방법으로 신속하게 일 처리하는 당신, 이미 인기쟁이군요! 🏄 타고난 공감 능력으로 상대방에게 깊게 공감할 수 있고, 애정 표현을 아끼지 않아요. 리더십이 있어서 애인이 더 좋은 사람이 되도록 이끌어주기도 하네요! 한편 당신이 도움을 받는 입장이 되면 마음이 무거워져요. 홀로 문제를 해결하려고 하기 보다, 사랑하는 이의 손을 붙잡는 건 어떨까요? 그가 당신의 손을 기다리고 있어요.',
            movieUrl: 'www.netflix.com/title/80203147',
            
            imageUrl: '/images/result/peter.png',
            imageSmallUrl: '/images/result/peter_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/YAPNSV.jpg',

            goodMbti: 'INFP',
            badMbti:'ESTJ',
            shortDesc: '센스 넘치는 사랑꾼 남자.\n당신을 아름답고 풍요로운 생활로 이끌어주지만, 때론 그의 고민을 들어보세요.',

            analysisTitle : "ENFJ 피터가 라라진을 사로잡은 비결💘",
            analysisSubTitle : "피터의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"피터 머릿속 33명의 기쁨이가 센스 넘치는 사랑꾼으로 만들었어요."\n캐릭터 평균 대비 4명↑)',
            emotion: [33, 20, 16, 12, 19]
           
        },
        female:{
            mbti : 'ENFJ',
            movie: '라라랜드',
            name: '미아',
            title: '센스 넘치는 사랑꾼 당신은,\n라라랜드의 미아',
            script: '"사람들은 다른 사람들의 열정에 끌리게 되어있어."',
            description: '솔직하고 센스 있는 ENFJ 유형은, 호불호가 명확한 편이에요. 말을 잘하고 부드러운 리더십도 있어 주위에 사람들이 많이 모이죠. 효율적인 방법으로 신속하게 일 처리하는 당신, 이미 인기쟁이군요! 🏄 타고난 공감 능력으로 상대방에게 깊게 공감할 수 있고, 애정 표현을 아끼지 않아요. 리더십이 있어서 애인이 더 좋은 사람이 되도록 이끌어주기도 하네요! 한편 당신이 도움을 받는 입장이 되면 마음이 무거워져요. 홀로 문제를 해결하려고 하기 보다, 사랑하는 이의 손을 붙잡는 건 어떨까요? 그가 당신의 손을 기다리고 있어요.',
            movieUrl: 'www.watcha.com/contents/m5GXz42',
            
            imageUrl: '/images/result/mia.png',
            imageSmallUrl: '/images/result/mia_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/Kqn1TC.jpg',

            goodMbti: 'INFP',
            badMbti:'ESTJ',
            shortDesc: '센스 넘치는 사랑꾼 여자.\n당신을 아름답고 풍요로운 생활로 이끌어주지만, 때론 그의 고민을 들어보세요.',

            analysisTitle : "ENFJ 미아가 세바스찬을 사로잡은 비결💘",
            analysisSubTitle : "미아의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"미아 머릿속 19명의 버럭이가 적극적인 사랑꾼으로 만들었어요."\n캐릭터 평균 대비 3명↑)',
            emotion: [25, 19, 19, 12, 25]
           
        }
    },
    INTJ:{ 
        male:{
            mbti : 'INTJ',
            movie: '오만과 편견',
            name: '다아시',
            title: '논리적인 사랑을 하는 당신은,\n오만과 편견의 다아시',
            script: '"더 이상 내가 싫지 않다면, 고백하건대 난 마법에 걸렸습니다. 당신을 사랑하고 또 사랑합니다."',
            description: '독립적으로 사고하고 행동하는 INTJ 유형은, 명확한 목표를 갖고 있어요. 생명력과 열정을 갖고 사는 사람에게 푹 빠지는 스타일이죠. 👨 💼 겉으론 무뚝뚝한 INTJ이기에 애정 표현을 하는 게 어색해요. 게다가 어딘가 어른스럽고 냉철해 보여서, 상대방도 거리감을 느껴요. 게다가 INTJ는 연애도 문제 해결의 과정처럼 느낀답니다. 그런데 애인은 당신이 사랑하는 방식에 낯설 수도 있어요. 생각이 관계를 가로막는 당신에게 해주고 싶은 말은, “너무 깊이 생각하지 말아요. 솔직하게 마음을 따라가보세요.“',
            movieUrl: 'www.watcha.com/contents/mOk6ZMW',
            
            imageUrl: '/images/result/darcy.png',
            imageSmallUrl: '/images/result/darcy_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/kT75nf.jpg',

            goodMbti: 'ENFP',
            badMbti:'ISFJ',
            shortDesc: '성숙하고 독립적인 남자.\n그의 지적인 행동 방식이 멋있기도 하지만, 가끔 이해할 수 없어요.',

            analysisTitle : "INTJ 다아시가 리지를 사로잡은 비결💘",
            analysisSubTitle : "다아시의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"다아시 머릿 속 37명의 기쁨이가 그녀에게 마음이 직진하도록 만들었어요."\n캐릭터 평균 대비 8명↑)',
            emotion: [37, 23, 17, 5, 18]
           
        },
        female:{
            mbti : 'INTJ',
            movie: '양들의 침묵',
            name: '클라리스',
            title: '논리적인 사랑을 하는 당신은,\n양들의 침묵 클라리스',
            script: '"배우러 왔습니다. 자격이 있는지 없는지는 직접 판단하세요."',
            description: '독립적으로 사고하고 행동하는 INTJ 유형은, 명확한 목표를 갖고 있어요. 생명력과 열정을 갖고 사는 사람에게 푹 빠지는 스타일이죠. 👩 💼 겉으론 무뚝뚝한 INTJ이기에 애정 표현을 하는 게 어색해요. 게다가 어딘가 어른스럽고 냉철해 보여서, 상대방도 거리감을 느껴요. 게다가 INTJ는 연애도 문제 해결의 과정처럼 느낀답니다. 그런데 애인은 당신이 사랑하는 방식에 낯설 수도 있어요. 생각이 관계를 가로막는 당신에게 해주고 싶은 말은, “너무 깊이 생각하지 말아요. 솔직하게 마음을 따라가보세요.“',
            movieUrl: 'www.watcha.com/contents/mW4L2XW',
            
            imageUrl: '/images/result/clarice.png',
            imageSmallUrl: '/images/result/clarice_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/caXAFg.jpg',

            goodMbti: 'ENFP',
            badMbti:'ISFJ',
            shortDesc: '성숙하고 독립적인 여자.\n그의 지적인 행동 방식이 멋있기도 하지만, 가끔 이해할 수 없어요.' , 

            analysisTitle : "INTJ 클라리스가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "클라리스의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"클라리스 머릿 속 26명의 슬픔이가 이성에게 감동을 받도록 만들었어요."\n캐릭터 평균 대비 6명↑)',
            emotion: [19, 26, 16, 13, 26]
           
        }
    },
    INTP:{ 
        male:{
            mbti : 'INTP',

            movie: '소셜 네트워크',
            name: '마크 주커버그',
            title: '사랑따윈 필요없는 당신은,\n소셜 네트워크의 마크 주커버그',
            script: '"저와 제 동료가 하는 일은 이 방에 있는 누구도 절대 할 수 없는 지적이고 창의적인 능력을 요구하는 일이거든요."',
            description: '자기애가 높은 INTP는 사랑도 분석적으로 해요. 사랑에 빠지고, 연애하고, 마음을 떠나는 것까지 명확하게 구분하고 느끼는 편이죠. ✂️ 독립적이고 분석적인 성격 탓에 이상형이 매우 확실한 편이라서, 연애를 시작하는 게 대부분 어려웠을 거예요. 사랑에 빠졌다고 해도, 콩깍지를 벗어나는 건 한순간이에요. 당신에게 사랑이 1순위가 아니니까요. 세상이 INTP를 연애에 적합하지 않은 성격이라고 말해요. 하지만 무BTI는 그렇게 생각해요. 세상엔 다양한 사랑의 형태가 있다. 당신의 사랑도 사랑이다.',
            movieUrl: 'www.netflix.com/title/70132721',
            
            imageUrl: '/images/result/mark.png',
            imageSmallUrl: '/images/result/mark_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/NfZEnh.jpg',

            goodMbti: 'ENTJ',
            badMbti:'ISFP',
            shortDesc: '분석의 끝에 사랑이 있는 남자. \n뇌섹남 매력을 갖고 있어요. 사랑이 1순위가 아니기에, 너무 깊게 빠지면 안돼요.',

            analysisTitle : "INTP 주커버그가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "주커버그의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"주커버그 머릿 속 25명의 버럭이가 강력한 차도남의 매력을 만들었어요."\n캐릭터 평균 대비 9명↑)',
            emotion: [26, 20, 25, 8, 21]
           

        },
        female:{
            mbti : 'INTP',
            movie: '인셉션',
            name: '아리아드네',
            title: '사랑따윈 필요없는 당신은,\n인셉션의 아리아드네',
            script: '"전 그저 당신이 매일 밤 어떤 테스트를 하는 지 보고 싶었어요."',
            description: '자기애가 높은 INTP는 사랑도 분석적으로 해요. 사랑에 빠지고, 연애하고, 마음을 떠나는 것까지 명확하게 구분하고 느끼는 편이죠. ✂️ 독립적이고 분석적인 성격 탓에 이상형이 매우 확실한 편이라서, 연애를 시작하는 게 대부분 어려웠을 거예요. 사랑에 빠졌다고 해도, 콩깍지를 벗어나는 건 한순간이에요. 당신에게 사랑이 1순위가 아니니까요. 세상이 INTP를 연애에 적합하지 않은 성격이라고 말해요. 하지만 무BTI는 그렇게 생각해요. 세상엔 다양한 사랑의 형태가 있다. 당신의 사랑도 사랑이다.',
            movieUrl: 'www.netflix.com/title/70131314',
            
            imageUrl: '/images/result/ariadne.png',
            imageSmallUrl: '/images/result/ariadne_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/lX1MHo.jpg',


            goodMbti: 'ENTJ',
            badMbti:'ISFP',
            shortDesc: '분석의 끝에 사랑이 있는 여자. \n뇌섹녀 매력을 갖고 있어요. 사랑이 1순위가 아니기에, 너무 깊게 빠지면 안돼요.', 

            analysisTitle : "INTP 아리아드네가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "아리아드네의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"아리아드네 머릿 속 25명의 소심이가 무뚝뚝한 차도녀의 매력을 만들었어요."\n캐릭터 평균 대비 2명↑)',
            emotion: [13, 31, 18, 13, 25]

        }
    },
    ENTP:{ 
        male:{
            mbti : 'ENTP',
            movie: '헤더스',
            name: 'J.D.',
            title: '사랑을 쟁취하는 당신은,\n헤더스의 J.D.',
            script: '"극단적인 건 항상 나에게 감흥을 주지."',
            description: '밝고 활동적인 ENTP는 자존심 강한 사랑꾼이에요. 자신과 찰떡이라는 사람이 등장하면, 당당하게 다가가 사랑을 쟁취하는 스타일이죠 🦸 자기가 좋아하는 스타일을 찍어서, 썸도 잘 타는 성격인 ENTP. 그야말로 자타공인 연애고수죠. 또 특이한 사람에게 강한 느낌을 받기도 해요. 썸 타고 연애하는 데 능숙하지만, 당신의 자유로운 성격 때문에 헤어질 위기를 겪은 적 없나요? 상대방이 당신의 자유로운 모습에 감당 못 할 수도 있고, 당신이 억압받는다 생각되는 순간 관계는 끝이 나요. 하지만 너무 슬퍼하지 말아요. 곧 당신에게 새로운 연인이 생길 거예요.',
            movieUrl: 'www.watcha.com/contents/mqW9045',
            
            imageUrl: '/images/result/jd.png',
            imageSmallUrl: '/images/result/jd_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/052kT5.jpg',

            goodMbti: 'INTJ',
            badMbti:'ESFJ',
            shortDesc: '당당하고 썸 잘타는 남자.\n낭만적 연애의 고수죠. 다만 당신이 자존심을 부리면 관계는 끝난답니다.',

            analysisTitle : "ENTP J.D.가 베로니카를 사로잡은 비결💘",
            analysisSubTitle : "J.D.의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"J.D. 머릿 속 24명의 버럭이가 당당하고 썸 잘타는 성격을 만들었어요."\n캐릭터 평균 대비 8명↑)',
            emotion: [14, 25, 24, 12, 25]
           

        },
        female:{
            mbti : 'ENTP',
            movie: '펄프픽션',
            name: '미아',
            title: '사랑을 쟁취하는 당신은,\n펄프픽션의 미아',
            script: '"난 저 트로피를 원해, 그러니까 춤을 잘 춰봐."',
            description: '밝고 활동적인 ENTP는 자존심 강한 사랑꾼이에요. 자신과 찰떡이라는 사람이 등장하면, 당당하게 다가가 사랑을 쟁취하는 스타일이죠 🦸 ♀️ 자기가 좋아하는 스타일을 찍어서, 썸도 잘 타는 성격인 ENTP. 그야말로 자타공인 연애고수죠. 또 특이한 사람에게 강한 느낌을 받기도 해요. 썸 타고 연애하는 데 능숙하지만, 당신의 자유로운 성격 때문에 헤어질 위기를 겪은 적 없나요? 상대방이 당신의 자유로운 모습에 감당 못 할 수도 있고, 당신이 억압받는다 생각되는 순간 관계는 끝이 나요. 하지만 너무 슬퍼하지 말아요. 곧 당신에게 새로운 연인이 생길 거예요.',
            movieUrl: 'www.watcha.com/contents/mOgBDaW',
            
            imageUrl: '/images/result/pulp_mia.png',
            imageSmallUrl: '/images/result/pulp_mia_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/f9UVib.jpg',

            goodMbti: 'INTJ',
            badMbti:'ESFJ',
            shortDesc: '당당하고 썸 잘타는 여자.\n낭만적 연애의 고수죠. 다만 당신이 자존심을 부리면 관계는 끝난답니다.' , 

            analysisTitle : "ENTP 미아가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "미아의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"미아 머릿 속 30명의 기쁨이가 당당하고 썸 잘타는 성격을 만들었어요."\n캐릭터 평균 대비 1명↑)',
            emotion: [30, 25, 14, 10, 21]
           
        }
    },
    ENTJ:{ 
        male:{
            mbti : 'ENTJ',
            movie: '그레이의 50가지 그림자',
            name: '그레이',
            title: '어느새 사랑에 스며든 당신은,\n그레이의 50가지 그림자의 그레이',
            script: '"내 세상의 시작과 끝은 당신과 함께였으면 좋겠어."',
            description: '탁월한 리더십을 갖춘 ENTJ 유형은, 마음속에 엄격한 기준을 품고 있어요. 하지만 막상 이상형보다는 자연스럽게 타인에게 빠져버리죠. 🎯 ENTJ는 연애도 목표 지향적이에요. 연애의 시작은 사람이 아니라, 관계의 맥락이에요. 예를 들면 그가 처음부터 좋은 게 아니라, 독서클럽에 가서 만난 그와 대화를 하면서 빠져드는 거죠. 완벽주의자라 스트레스를 많이 받고, 그 스트레스 때문에 연인과 관계가 안 좋아질 수도 있어요. 이제 편하게 마음을 내려놓고, 그에게 기대보세요.',
            movieUrl: 'www.netflix.com/title/80131194',
            
            imageUrl: '/images/result/gray.png',
            imageSmallUrl: '/images/result/gray_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/NtciVS.jpg',

            goodMbti: 'INTP',
            badMbti: 'ESFJ',
            shortDesc: '탁월한 리더십을 갖춘 남자. \n당신과 대화하며 교류하는 순간에 자기도 모르게 푹 빠져버려요.',

            analysisTitle : "ENTJ 그레이가 사랑을 쟁취하는 비결💘",
            analysisSubTitle : "그레이의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"그레이 머릿 속 40명의 기쁨이가 어느새 그녀에게 빠지게 만들어요."\n캐릭터 평균 대비 11명↑)',
            emotion: [40, 20, 11, 9, 20]
           
        },
        female:{

            mbti : 'ENTJ',
            movie: '친구와 연인사이',
            name: '엠마',
            title: '어느새 사랑에 스며든 당신은,\n친구와 연인 사이의 엠마',
            script: '"그 사람 생각을 멈출 수가 없어."',
            description: '탁월한 리더십을 갖춘 ENTJ 유형은, 마음속에 엄격한 기준을 품고 있어요. 하지만 막상 이상형보다는 자연스럽게 타인에게 빠져버리죠. 🎯 ENTJ는 연애도 목표 지향적이에요. 연애의 시작은 사람이 아니라, 관계의 맥락이에요. 예를 들면 그가 처음부터 좋은 게 아니라, 독서클럽에 가서 만난 그와 대화를 하면서 빠져드는 거죠. 완벽주의자라 스트레스를 많이 받고, 그 스트레스 때문에 연인과 관계가 안 좋아질 수도 있어요. 이제 편하게 마음을 내려놓고, 그에게 기대보세요.',
            movieUrl: 'www.netflix.com/title/70159317',
            
            imageUrl: '/images/result/emma.png',
            imageSmallUrl: '/images/result/emma_small.png',
            kakaoImageUrl: 'https://ifh.cc/g/aU5tDG.jpg',

            goodMbti: 'INTP',
            badMbti: 'ESFJ',
            shortDesc: '탁월한 리더십을 갖춘 여자. \n당신과 대화하며 교류하는 순간에 자기도 모르게 푹 빠져버려요.',

            analysisTitle : "ENTJ 엠마가 아담을 사로잡은 비결💘",
            analysisSubTitle : "엠마의 대사를 분석해서 \n 머릿 속 감정 친구들 100명을 찾아냈어요!",
            analysisText : '"엠마 머릿 속 18명의 까칠이가 완벽주의자 차도녀 매력을 만들어요."\n캐릭터 평균 대비 6명↑)',
            emotion: [25, 18, 19, 18, 20]
           
        }
    },
}


export { resultList }
